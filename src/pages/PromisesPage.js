...
const PromisesPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (newPromise.type === 'text' && !newPromise.content.trim()) return;
    ...
    const promise = {
      id: Date.now(),
      content: content,
      description: newPromise.type !== 'text' ? newPromise.description : "", // 保存描述
  ...
  const renderPromiseForm = () => {
    ...
    return (
      <form onSubmit={handleSubmit}>
        ...
        {(newPromise.type === 'text' || uploadedFile) && (
    ...
    if (promise.type === 'text') {
      ...
      return (
        <div className="promise-media">
          ...
          {promise.fileName && <div className="file-label">文件: {promise.fileName}</div>}
          {promise.description && (
