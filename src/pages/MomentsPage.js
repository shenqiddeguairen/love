...
const MomentsPage = () => {
  ...
  const handleFileUpload = (e) => {
    ...
    } else if (newMoment.type === 'text' && (file.type.includes('text') || file.type.includes('document'))) {
      // 读取文本文件内容
  ...
  const handleSubmit = (e) => {
    ...
    if (newMoment.type === 'text' && !newMoment.content.trim()) return;
  ...
  const renderMomentForm = () => {
    ...
    return (
      <form onSubmit={handleSubmit}>
        ...
        {(newMoment.type === 'text' || uploadedFile) && (
  ...
  const renderMomentContent = (moment) => {
    if (moment.type === 'text') {
      return <div className="moment-content">"{moment.content}"</div>;
    } else if (moment.type === 'audio') {
      return (
        <div className="moment-media">
          <audio controls src={moment.content} className="audio-player"></audio>
          {moment.fileName && <div className="file-label">文件: {moment.fileName}</div>}
          {moment.description && (
